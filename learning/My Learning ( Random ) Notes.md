...
### Update Release File For GitHub Actions

Here is the full updated `release.yml` file that we need in order to be able to compile the new `graphs.exe` with a smaller file size.
...
jobs:
  build-and-release:
    ...
    steps:
      ...
      - name: Show executable sizes
        shell: powershell
        run: |
          Get-Item program.exe | Select-Object Name,Length
          Get-Item dist/graphs.exe | Select-Object Name,Length
...
> [!INFO] Resource(s)
> 
> - Official Python Documentation: https://docs.python.org/3/library/ctypes.html#module-ctypes
> - `GetSystemMetrics` Windows API Documentation: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getsystemmetrics

> [!NOTE]
> From what I can understand that `ctypes` Python module is basically used to access **low level C code** or **Windows API** related stuff.

So right now, we are have the following line in our heatmap function:
...
The above line basically get the *maximum* and *minimum* value for the `x` and `y` coordinates and then basically writes the minimum and maximum value on **both** the $x$ and $y$ axes.

I now want to make the axes actually represent the user's screen meaning if the user has a screen resolution of '1920x1080'... I want the $x$ axis to go from '0' to '1920' and also for the $y$ axis; I want to have '0' to '1080'.

Therefore, we need a way to get the screen resolution of the user's computer.

> [!WARNING]
> I don't know anything about the `ctypes` module in Python.
> 
> The code below was generated by Claude and I was talking to my friend about how I needed to do this bit for my project and we started chatting about it and google some stuff, talked with *his* [DeepSeek](https://chat.deepseek.com).
> 
> Additionally, if you search for the actual function that get's us the **length** and **width** of the screen. It does **not** show up in the Python documentation.
> 
> > What I am trying to say its that I fully used AI's code and played with it a little bit.
...
- This is the output after running the above *test* program:

> Given that I am on '1920x1080'... I should get `(1920, 1080)` as output right?
...
> [!WARNING]
> Our **displays scaling** is fucking us up!
> 
> Yes, currently I am on using 125% scaling on my display running the same code from above, but *now* using a display scaling of 100%... I get "*correct*" output.
> 
> ```console
> (1920, 1080)
> ```

### Fix: Ignore Display Scaling

> [!INFO] Resource(s)
> 
> - `SetProcessDPIAware` Windows API Documentation: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setprocessdpiaware 

Therefore, talking with my friend's DeepSeek... I asked it to generate a formula of some sort so that we are able to get the user's screen size correctly.

But instead it told us to add the following line just above the `width` and `height`.

> Therefore the function now looks like this.
...
## Make The Heatmap Graph More 16:9 Aspect Ratio

Currently the actual heatmap is just a simple square and if your mouse if near the middle of the screen but **more** to either sides... Then the graphs does not actually represent it well as its just a square there.

> Similar to how you have to map a specific region on a small Wacom tablet!

Therefore ( *and being honest here* ), I asked Claude on how we can actually modify it so that we have the heatmap more 16:9 aspect ratio like.

Therefore it told me to modify the following line **to this**:
...
> I am also going to add the following `figsize = (12.8, 7.2)` to our `barchart_generation`!

---

> [!NOTE]
> Please refer to the actual `main.py` file for the implementation.
...
>
> I actually change `ctypes.windll.user32.SetProcessDPIAware()` to this: 
> 
> ```python
>   # ignore display scaling ==> get the actual pixel coordinates of the screen
...
> [!INFO] Resource(s)
> - My Notes: [[Raylib and RayGUI ( Windows )]]

## Make A Theme Switcher Button

> [!INFO]
> - Andrew Hamel Codes: https://www.youtube.com/watch?v=OPkSJqVyP2w

After watching the above video and seeing how he coded it; I managed to make the following:
...
int main(void) {
  ...
  // load the dark theme downloaded
  GuiLoadStyle("style_dark.rgs");

  // load the colour into a variable
  ...
  while (!WindowShouldClose()) {
    ...
    {
      Color darkTheme = GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR));
      ClearBackground(currentTheme);

      if (GuiButton((Rectangle){0, 0, 100, 50}, "Change Theme")) {
    ...
    GuiLabel((Rectangle){100, 100, 100, 100}, "Testing");
...
It does work but there is a little something that is bothering me. So when you loaded the `style_dark.rgs` file it should also change the **font style** automatically but in my case its not going that!

> How do I know that: https://raylibtech.itch.io/rguistyler!

Therefore, I asked Gemini for help and it's code does indeed work.
...
```C
    ...
    while (!WindowShouldClose()) {
        ...
        // Dynamically fetch the background color directly from the active theme!
        // This will automatically be dark when style_dark is loaded,
        // and automatically light when the default style is loaded.
        Color currentBg = GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR));
        ClearBackground(currentBg);

        // The Magic Toggle Button
        if (GuiButton((Rectangle){ 150, 150, 200, 50 }, "Toggle Theme")) {

            // Flip our boolean state switch
            isDarkTheme = !isDarkTheme;

            if (isDarkTheme) {
                // Load the dark theme colors and its native default dark font
                GuiLoadStyle("style_dark.rgs");
            } else {
                // Completely restore Raygui's original light theme and original font!
                GuiLoadStyleDefault();
            }
        }

        // This label will now perfectly switch its colors AND its default font styles
        // depending on which theme bundle is currently loaded active.
        GuiLabel((Rectangle){ 150, 230, 200, 30 }, "Testing Default Typography");
...
> [!WARNING]
> What is the difference between *Retained Mode GUI * and *Immediate Mode GUI*?
> 
> Well, given that I come from the land of Java Swing, JS and things like that whereby we have something called an **Event Handler**.
> 
> Whereby it's simply a function that when, for example, a button press will actually perform and action.
> 
> But as you can see from the above code, we are running everything inside of a `while` loop; this means that everything is getting rendered all at once ( *like a game* )!
> 
> Hence, there is a mindset-shift that I need to do so that I am able to understand it.
> 

> [!WARNING]
> Given that the current way to get documentation for 'raygui' is to simply check the `raygui.h` *header file*... I am going to use Gemini in order to learn how do work with raygui while also making the things that I want to make.
> 
> > I know the use of AI is increasing!

> [!INFO]
> 
> A couple of things.
> 
> 1. I downloaded the style `.rgs` file from the following link: https://github.com/raysan5/rguistyler/tree/master/styles
> 2. I placed the `style_dark.rgs` file at the root of my `learning` folder ( *next to `main.c`* )

### Correct Way For "Event Handling"

> Again, I am learning with Google's Gemini!

> [!NOTE]
> I am only going to be showing you snippets of code.
...
```C
  while (!WindowShouldClose()) {
    ...
    if (GuiButton((Rectangle){0, 0, 120, 60}, isDarkTheme ? "Light Theme" : "Dark Theme")) {
...
> [!WARNING]
> I am programming freely and doing it on my as I think I am understanding it.
> 
> > Meaning that its not stuff that is being purely generated by Gemini...
> 
> Basically what I am trying to say its that I am having fun Googling around for solutions, coding it and making things display on my screen.
> 
> Therefore, I won't really be adding stuff here. Hence, I suggest you simply take a look at the actual code found on the repository and check it out.

---

> Just a thing that I want to write as my tiny brain was able to figure it out by myself.

So, we create an **integer**, we know that in terms of binary we are going to start from the right ( *like oriental languages like Arabic, etc* )... We also know that an **integer** is *4 bytes*.

Now, if we create a **float** value, we also know that its *4 bytes*... But what's the difference? **Mantissa** and **Exponent**... Well, I learned about those in Grade 11 ( *I think and completely forgot how to calculate them BTW* ).

This means that the way that they are structured in terms of *1's and 0's* are completely different.

Playing with the `GuiSliderBar`, the **fourth** argument requires `float *`... Well, I was thinking that I could just type cast my `int` and voila access that the reference.

> But wait a second!

Well, at that *memory address*, its **still an integer number**!

> That's all that I want to say!

---

# Integrating GUI Into Project

> [!NOTE]
> I would suggest you to re-clone the whole raylib repository and also re-download ( *or clone* ) the `raygui.h` header file.
> 
> This way, we know that everything is going to work correctly.
> 
> > But I am **not** going to do that as I know that everything is working fine, right now!

## Move Files And Folders Into mouse-c-py Project
...
Mode                 LastWriteTime         Length Name
----                 -------------         ------ ----
d-----         6/14/2026   2:25 PM                .github
d-----         6/14/2026   2:25 PM                assets
d-----         6/14/2026   2:25 PM                code learning
d-----         6/14/2026   2:25 PM                learning
d-----         6/14/2026   2:25 PM                raylib-windows
-a----         6/14/2026  12:35 PM            215 .gitignore
-a----         6/14/2026  12:35 PM          15717 main.c
-a----         6/14/2026  12:35 PM           9187 main.py
-a----         6/14/2026  12:35 PM            462 Makefile
-a----         6/14/2026  12:35 PM           5923 README.md
-a----         6/14/2026  12:35 PM            736 requirements.txt
```

- Move Raylib's header files and library files ( *and related files like `style_dark.rgs`* ) to `mouse-c-py` project:
...
d-----         6/14/2026   2:25 PM                assets
d-----         6/14/2026   2:25 PM                code learning
d-----         6/14/2026   3:06 PM                include
d-----         6/14/2026   2:25 PM                learning
d-----         6/10/2026   1:30 PM                lib
d-----         6/14/2026   2:25 PM                raylib-windows
-a----         6/14/2026  12:35 PM            215 .gitignore
-a----         6/14/2026  12:35 PM          15717 main.c
-a----         6/14/2026  12:35 PM           9187 main.py
...
-a----         6/14/2026  12:35 PM           5923 README.md
-a----         6/14/2026  12:35 PM            736 requirements.txt
-a----         6/12/2026  10:06 AM           3730 style_dark.rgs
```

## Update The Makefile

- Here is the new, updated `Makefile`:

```make
# remote - GitHub Actions ( compiling the `main.c` file )
build:
	@gcc main.c -Iinclude -Llib -lraylib -lgdi32 -lwinmm -lopengl32 -lshell32 -lpathcch -Wall -Wextra -pedantic -o program.exe
...
> [!BUG]
> > Why are you so shit Windows ( *specially for beginner programmers like me* )?
> 
> The current problem that *we* ( *because I know that I cannot solve this without the help of Claude* ) are trying to solve is the inclusion of **header files** of `raylib.h` and `windows.h`.
> 
> In the `windows.h` **header file**, there is this very `Rectangle` function:
> 
> ```C
...
> 
> > [!NOTE]
> > This line of code is found at line *3251* inside the `wingdi.h` header file... Which is what our `windows.h` header file is calling ( *from that `wingdi.h`* ).
> 
> Well, in our `raylib.h` file, we also have this:
> 
> ```C
...
> ```
> 
> This means that we get a **conflict** error whereby `windows.h` is **not** agreeing to having the new `Rectangle` structure found in our `raylib.h` file that we are actively using for our GUI.
> 
> Therefore, I am going to use Claude to solve the following issue.

> [!WARNING]
> Given the amount of time that I have; I am going to lean into more into the vibe-coding side of things as I want to also move onto other projects that I think is more interesting and will be of more value to me.

### Solving The Issue Of Header Files

> [!NOTE]
> The version of Raylib that I clone and built on my machine was **v6.0**.

> I don't know if this is going to work or not but let's try!

Now, Claude is telling us to let the `gui.c` and `main.c` file separate and link them together with our custom header files.

- Create a `win32_input.h` header file:

```C
#ifndef WIN32_INPUT_H
#define WIN32_INPUT_H

// shared state between win32_input.c and gui.c
// volatile because gui.c reads these while win32_input.c writes them
extern volatile double g_elapsed_time;
extern volatile int    g_is_logging;

// logging duration set by the GUI slider
...
> Now what the fuck is `extern volatile`?

> [!INFO] Resource(s)
> 
> - Medium Article: https://medium.com/@kulasekaranslrk/mastering-c-static-vs-inline-vs-const-vs-extern-vs-volatile-e4d858052efe
> - StackOverflow: https://stackoverflow.com/a/14333841

- Create the actual `win32_input.c` file:

> [!WARNING]
> I recommend you to take a look at the code over at the GitHub repository as its basically a *re-write* of my original `main.c` file but just manipulated most variables to be `static` so that its **not** seen by `gui.c`!

### What Is and Enums?

> [!INFO] Resource(s)
> 
> - W3 Schools: https://www.w3schools.com/c/c_enums.php
> - Reddit: https://www.reddit.com/r/C_Programming/comments/1cojqci/what_is_the_point_of_enums/
> - Bro Code: https://www.youtube.com/watch?v=sU0XQ3jSsu8

> So basically from what I can understand, they are constants that can be grouped and used together.

---

> [!INFO]
> Claude created a solution and after I copied stuff around and made the changes it requested... **It works**!
> 
> Therefore, I am going to redo these steps so that I am able to write notes about it here.

# Solving The Inclusion Issues

> It's actually what I kind-of wanted to be honest; to split into multiple files

Originally, we were trying to modify the original `main.c` file to *include* Raylib.

But after finding out that we have a lot of conflicts between the Raylib's GUI "*Windows*" functions and the `windows.h` actual Windows GUI functions.

Therefore, instead of trying to solve these issue ( *I mean its been a while since I got an issue like this* )... I basically decided that; yeah we should simply code them in different files ( *separate logger and GUI files* ).

In this case, we can have our `windows.h` hidden window that is used by Raw Input and while still using our `Rectangle` *structure* for our GUI.

> Hence, these are the changes that Claude asked me to make so that we can "*link*" them up and actually compile the file

## Getting Started

### Raylib and Raygui Header File

So, given that I had already built the `libraylib.a` library file by cloning the actual 'raylib' repository and running `make PLATFORM=PLATFORM_DESKTOP` inside the `src` folder.

Then for the `raygui.h` file, I mean I originally, did a `git clone` the 'raygui' repository but only cloned the `src` folder whereby the actual `raygui.h` file is stored!

Then, I created a folder inside the `mouse-c-py` directory called `include`, then added the **header files** inside of it.

Similar thing for the library file; whereby I created a `lib` folder inside the `mouse-c-py` directory and add the `libraylib.a` **library file** to the `lib` folder.

Additionally, I have download the `style_dark.rgs` file and placed it inside the `mouse-c-py` directly.

## GUI File

So from my learning over at '[[Raylib and RayGUI ( Windows )]]' and my little stream that I did over on '[YouTube](https://www.youtube.com/live/Ul96eFcTSu8?si=wAcytlD5K30rMfSi)'.

> Like I was saying before...

I was able to code most ( *if not all* ) of the structure and placed all my *labels* and *buttons*. For some button, like the "*theme*" button, I added the functionality while the *start* and *stop* logging button I left it for later ( *whereby I let Claude vibe-code* ) it.

## GUI and Mouse Logger

Therefore, given our issue with the `raylib.h` and `windows.h` **header files**... We decided that the best approach would be to simply keep them **separately** and simply *link* them up when compiling.

Therefore,  for the mouse logging and "*writing-to-file*" part, we created a `win32_input.h` and hence, its `win32_input.c` file which is basically a remake of our **current** `main.c` file split into little functions whereby we are able to call inside our `gui.c` file!

Given that we now have **2** files that we 2 `.c` files; well we need to update our `Makefile` so that we are able to link

### Makefile

- Here is how our **updated** `Makefile` now looks like:

```make
# remote - GitHub Actions ( compiling the `main.c` file )
build:
	@gcc win32_input.c gui.c -Iinclude -Llib -lraylib -lgdi32 -lwinmm -lopengl32 -lshell32 -lpathcch -Wall -Wextra -o program.exe
...
### Updated Mouse Logger Part

So we split up the files so that our `windows.h` and `raylib.h` header files does **not** conflict with each other.

Hence, we created the following `win32_input.h` header file.
...
> [!NOTE]
> I am **not** going to go over all of these individual functions ( *if at all* ).
> 
> Please do head over the GitHub repository to view the actual `win32_input.c` and `gui.c` files.
